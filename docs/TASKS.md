...
resetTasks();
// 1) отменяет все ожидающие встроенные when и retry-delay
// 2) инвалидирует поздние результаты function-when и Task.run
// 3) очищает done и cache → стадию можно запустить снова
```
...
| Действие | Поведение |
|----------|-----------|
| `timeout:*` / `data:*` | таймер/interval снимается, `run` не вызывается |
| `port:*` / `allPorts:*` / `worker:*` | подписки снимаются, `run` не вызывается |
| `load` / `visible` / `custom:*` | DOM-listener снимается, `run` не вызывается |
| `idle` | idle callback или fallback-таймер отменяется, `run` не вызывается |
| ожидающий `retry` | задержка отменяется, следующая попытка не запускается |
| функция-`when` | Promise физически не отменяется, но его поздний результат игнорируется |
| `done` / `cache` | очищаются |
| уже запущенный `Task.run()` | не прерывается; поздний успех не записывает `done`, поздняя ошибка не запускает retry |

Каждый сброс начинает новое поколение lifecycle. Благодаря этому результат пользовательского
Promise из предыдущего поколения не может изменить новое состояние, даже если сам Promise
невозможно отменить физически.

> Полезно в hot‑reload, когда нужно заново проиграть стадию. Дождитесь завершения текущего
> `runTasks()`, прежде чем запускать её снова: защита от двух одновременных запусков сохраняется.
