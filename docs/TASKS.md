...
resetTasks();
// 1) отменяет ожидающие when: timeout:* и data:* (condition → false, Task.run не вызовется)
// 2) очищает done и cache → стадию можно запустить снова
```
...
| Действие                                            | Поведение                                                  |
|-----------------------------------------------------|------------------------------------------------------------|
| `timeout:<ms>` в полёте                             | таймер снимается, условие = `false`, `run` не вызывается   |
| `data:<key>` polling                                | interval снимается, условие = `false`, `run` не вызывается |
| `done` / `cache`                                    | очищаются                                                  |
| `port:*` / `allPorts:*` / `worker:*` уже в ожидании | **подписка пока остаётся** до первого события (см. ниже)   |
| `load` / `idle` / `visible` / `custom` в ожидании   | **не отменяются** этим вызовом                             |
| уже запущенный `Task.run()`                         | **не прерывается**                                         |

> Полезно в hot‑reload, когда нужно заново проиграть стадию.

> Ограничение: если `runTasks` ждёт `port:*` / `worker:*` / `allPorts:*` и вы вызвали `resetTasks()` **до** события, listener ещё может сработать позже и задача всё равно выполнится. Для timeout/data такого уже нет.
