...
Если callback `watch` или `subscribe` выбрасывает исключение, оно не подавляется, но уже подтверждённая мутация снимается с очереди и последующие мутации продолжают выполняться.
