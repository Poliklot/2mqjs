...
### Порядок мутаций и updater-функции

* `set`, `update`, `merge`, `add`, `remove` и `del` попадают в общую FIFO-очередь в порядке вызова.
* Store ждёт завершения hydration и `ready`, затем отправляет сериализируемые мутации worker-у конвейером в порядке вызова. Обычные `set(value)`, `merge`, `add`, `remove(value)` и `del` не ждут round-trip между собой.
* Updater-функции из `set(path, updater)`, `update(path, updater)` и predicate из `remove(path, predicate)` остаются на главном потоке: функции нельзя передавать через `postMessage`.
* Updater исполняется, когда доходит до функционального барьера и все предыдущие операции подтверждены. Он получает значение из последнего подтверждённого worker state. Поэтому последовательные вызовы не теряют изменения:

```ts
store.update('count', count => Number(count ?? 0) + 1);
store.update('count', count => Number(count ?? 0) + 1);
// Подтверждённые состояния: count = 1, затем count = 2.
```

Мутирующие методы возвращают `void` сразу после постановки в очередь. Обычная операция после updater может быть отправлена worker-у сразу: порядок применения сохраняет сам Worker. Следующий updater ждёт подтверждения всех операций перед ним. `get()` возвращает последний уже подтверждённый снимок и не ждёт ещё не выполненные мутации. Для реакции на их завершение используйте `watch` или `subscribe`: уведомления приходят по подтверждённым состояниям в FIFO-порядке.

Если callback `watch` или `subscribe` выбрасывает исключение, оно остаётся видимым вызывающему окружению, но уже подтверждённая мутация снимается с очереди и последующие мутации продолжают выполняться.
...
* Main thread использует FIFO-конвейер: fast path отправляет обычные мутации без лишнего ожидания, а updater/predicate-барьеры вычисляются только из подтверждённого canonical state worker.
